...
QUERY_TYPE_SUFFIX = {
    "factual": "TRẢ LỜI ĐẦY ĐỦ VÀ CHI TIẾT. Liệt kê chính xác toàn bộ các con số, mốc thời gian, tên gọi hoặc các trường hợp ngoại lệ có trong ngữ cảnh. Tránh lược bỏ thông tin.",
    "procedural": "Liệt kê chi tiết các bước thực hiện. Mỗi bước cần ghi rõ hồ sơ, địa điểm hoặc các yêu cầu cụ thể đi kèm.",
    "conditional": "Liệt kê đầy đủ mọi điều kiện, tiêu chuẩn hoặc đối tượng áp dụng. Sử dụng gạch đầu dòng để làm rõ từng ý, bao gồm cả các trường hợp đặc biệt.",
    "synthesis": "Tổng hợp và trả lời ĐẦY ĐỦ tất cả các vế của câu hỏi. Nếu có mâu thuẫn giữa các văn bản, hãy nêu rõ cả hai hoặc ưu tiên văn bản mới nhất nhưng giữ lại các chi tiết cụ thể.",
}
...
R2. **ƯU TIÊN TÍNH ĐẦY ĐỦ:** Câu trả lời phải bao phủ toàn bộ các chi tiết quan trọng (con số, thời gian, danh sách loại, ngoại lệ). Không được cắt bớt thông tin chỉ để ngắn gọn.
R3. **TRÍCH DẪN CHI TIẾT:** Khi ghi các mốc thời gian hoặc hệ số, phải ghi ĐẦY ĐỦ (Ví dụ: 'Tiết 13 (18:00-18:50)', 'Hệ số 3 với lý thuyết, hệ số 1 với tiểu luận').
R4. **XỬ LÝ MÂU THUẪN (RẤT QUAN TRỌNG):** Ưu tiên quy định mới nhất. Tuy nhiên, nếu quy định mới (ví dụ QĐ 1150) chỉ nói chung chung về một đối tượng, mà quy định cũ (ví dụ QĐ 1146) có định nghĩa chi tiết (ví dụ: các loại danh hiệu 'Xuất sắc', 'Giỏi'), bạn PHẢI sử dụng định nghĩa chi tiết đó để trả lời.
R5. **CẤU TRÚC RÕ RÀNG:** Sử dụng gạch đầu dòng cho danh sách. Đưa thông tin quan trọng nhất lên đầu. Không dùng câu chào hay câu dẫn rườm rà.
R6. **ĐỐI CHIẾU BẢNG:** Khi đọc bảng Markdown, luôn đối chiếu giá trị ô với tiêu đề cột tương ứng. Cẩn thận với các ô trống (thông tin thường nằm ở các dòng phía trên).
