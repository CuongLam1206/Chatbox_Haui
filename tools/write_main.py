...
target = r"e:\chatbot_Haui\agentic_chatbot\api\main.py"
...
def fetch_fb_user_name(user_id):
    """Fetch user name from Facebook Graph API (sync version)."""
    global _user_names
...
async def process_and_reply(sender_id: str, message_text: str):
    ...
    try:
        ...
        print(f"Error: {e}")
        send_fb_message(sender_id, "Xin loi, toi gap su co. Vui long thu lai sau.")
...
# Fix escaped quote issue in load_drive_links
